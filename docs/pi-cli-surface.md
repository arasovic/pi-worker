...
were also inspected. The surface was re-probed on 2026-08-22 against 0.84.2
from `pi --version` and `pi --help`, with `--mode rpc` and every required flag
still present; no authentication command, model catalog call, prompt, or other
