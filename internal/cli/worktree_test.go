...
// prepareWorktree is a test-only helper that delegates to the
// worktree package and converts Prepared into the (path, branch, err)
// tuple the CLI-facing tests expect.
func prepareWorktree(ctx context.Context, cwd, name string) (path, branch string, err error) {
	p, err := worktree.Prepare(ctx, cwd, name)
	return p.Path, p.Branch, err
}
