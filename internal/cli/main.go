...
// aggregate run result. The decision lives in run.RunFailure, the one
// place that reads a result's own fields, so a background run persisting
// its outcome and this CLI reporting its exit word cannot disagree.
func runFailure(result run.Result) (contracts.RunStatus, *contracts.RunError) {
	return run.RunFailure(result)
}
