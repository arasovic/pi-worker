...
func (w *DefaultWorker) Run(ctx context.Context, req WorkerRequest) (result WorkerResult) {
	...
	if transcript.assistantError() {
		...
		// field is provider-controlled prose and may carry secrets,
		// credentials, URLs, or unstable response bodies. An error stop
		// is not a completed answer even if a provider emitted some text;
		// withThinking preserves any such streamed text as partialExplanation.
		return withThinking(WorkerResult{Model: req.Model, Status: StatusFailed, Error: "upstream provider returned an error before producing final text"})
	}
