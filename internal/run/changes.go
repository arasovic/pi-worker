...
type FileChange struct {
	...
	// Directory is true when the changed path is itself a
	// directory: a defensive / legacy directory-shaped manifest entry
	// carrying zero added and deleted line counts. Collapsed nested
	// repositories are rejected before a manifest is produced, so they
	// never reach this field; the field remains solely for consumer
	// contract compatibility (older JSON schemas expect the key). The
...
// command per file up to the cap. If any trailing-slash untracked path
// carries a .git marker, the entire measurement aborts (fail-closed
// rule #187) — it returns an error rather than emitting a directory
// entry or attempting a no-index comparison. This check uses Lstat,
// does not recurse into the path, enter the repository, or run nested
// Git. Every command is read-only: no add
...
//
// The listing shapes the manifest handles are git's own shapes.
// Git collapses an untracked nested repository to one trailing-slash
// entry only while nothing about the directory is known to the index;
// a tracked-path replacement — a tracked directory replaced by a
// nested repository, or a nested repository within a tracked directory
// that still has tracked files — causes git to descend and list inner
// files individually. A nested repository that replaces a tracked file
// of the same name is reported as a tracked modification with no
// untracked listing. The manifest records each shape as git produces
// it: inner files when git lists them through a tracked-path
// replacement, and ordinary entries for everything that survives the
// .git-marker rejection.
func measureChangeFiles(ctx context.Context, root, head string, dirtyStamps map[string]fileStamp) (*Changes, error) {
	...
	if err := rejectCollapsedNestedRepositories(root, untrackedRaw); err != nil {
		return nil, err
	}
	// Trailing-slash entries in git's raw output are now rejected by
	// rejectCollapsedNestedRepositories (which checks for .git markers)
	// before reaching here, so every remaining path is an untracked file
	// or a directory without an embedded repository. Every path is
	// canonicalised by dropping the trailing slash that indicates a
	// directory shape, because the same root-relative path is the same
	// path regardless of whether git listed it as a directory entry:
	// a path that was a regular file when the dirty snapshot ran and
	// became a directory by measurement time must merge into one entry.
	// This normalization keeps stamp keys and measurement paths aligned.
	...
	// dirty union term is not optional: without it a worker that reverts
	// an already-dirty file to its committed content drops out of the
	// HEAD diff completely and the manifest would go silent even though
	// the caller's uncommitted work was destroyed.
	...
	for path, stamp := range dirtyStamps {
		...
		if stamp.descendants == nil {
			// A defensive directory stamp with no tracked descendant set
			// survives unchanged if it still appears in the after
			// listing. Directory presence in the listing is the only
			// identity available at the workspace level for these
			// shapes; a directory stamp whose path is gone from the
			// after listing is not unchanged and must be reported.
	...
	for _, path := range keptUntracked {
		// Registered submodules and pre-existing directory-shaped
		// untracked entries arrive here. Ordinary untracked directories
		// git enumerates alongside their files also arrive because their
		// names appear separately in the untracked slice after dedup.
		info, err := os.Lstat(filepath.Join(root, path))
		if err != nil {
	...
	// marker, so removing a pre-existing directory-shaped dirty path is
	// reported as a deleted directory entry, not as a deleted file.
	// Pre-existing collapsed nested repositories now fail before the
	// worker runs, so only tracked-path or defensive non-repository
	// directory shapes reach this code. Current presence decides first: a
...
// which is a legitimate dirty state.
// A tracked path that resolves to a directory, or a defensive
// non-repository directory entry, gets a directory stamp instead.
// Registered submodules are ignored, and collapsed nested repositories
// are rejected before stamps are produced.
// Size plus
...
type fileStamp struct {
	...
	// tracked-path directory stamp, or nil when no tracking information
	// existed at that path (untracked directory without known containment).
	descendants []string
...
// and never by entering another repository: git ls-files does not
// descend into nested repositories, so no .gitignore inside one is
// listed, and a nested repository that git enumerates file-by-file
// contributes only the entries git itself names. Three listings cover every rule file
...
// Collapsed nested repositories are rejected before any stamp exists.
// The
...
func snapshotDirtyStamps(ctx context.Context, dir string) (map[string]fileStamp, error) {
	...
	// string, which is its content — can say it moved.
	// Tracked paths that resolve to directories and defensive
	// non-repository directory entries carry a directory stamp instead.
	for _, raw := range append(trackedRaw, untrackedRaw...) {
		// Stamp keys are canonicalised by stripping the trailing slash
		// git emits for directory shapes; the stripped form is what every
		// measurement path uses, so stamp keys and measurement paths
		// agree byte for byte and the subtraction never silently keys
		// against nothing.
...
// rejectCollapsedNestedRepositories checks each trailing-slash
// entry in git's raw untracked listing for a .git marker using Lstat;
// only trailing-slash entries are examined, so plain file names pass
// regardless of what sits on disk. It does not recurse into paths,
// enter repositories, or run nested Git. The function returns nil
// when no trailing-slash entry names a nested repository.
func rejectCollapsedNestedRepositories(root string, rawPaths []string) error {
	for _, entry := range rawPaths {
		if !strings.HasSuffix(entry, "/") {
			continue
		}
		path := strings.TrimSuffix(entry, "/")
		present, err := gitMarkerPresent(root, path)
		if err != nil {
			return fmt.Errorf("%s/.git: %w", path, err)
		}
		if present {
			return fmt.Errorf("nested repository %q cannot be measured", path)
		}
	}
	return nil
}
