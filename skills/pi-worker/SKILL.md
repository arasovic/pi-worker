...
- Use trusted workspaces. Parallel writes must be disjoint, and whenever `--writes` is used, one run at a time per workspace: the write check cannot tell one run's changes from another's.
